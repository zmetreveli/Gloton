...
const Restaurante = require("../schema/RestauranteSchema");
const User = require("../schema/usersSchema");
const { encryptValue } = require("../utils"); // bcrypt no hace falta aquí

// Crear restaurante nuevo (solo si el usuario es RESTAURANT)
exports.createRestaurante = async (req, res) => {
  const idUser = req.params.idUser;
...
// ✅ Obtener TODOS los restaurantes de la BBDD (para la HomePage)
exports.getRestaurantes = async (req, res) => {
  console.log("📥 Controller: getRestaurantes");

  try {
    const restaurantes = await Restaurante.find(); // aquí solo leemos BBDD
